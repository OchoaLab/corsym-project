...
The desired correlation for this data should not depend on the order of the parent values, which is arbitrary.
However, Pearson correlation estimates can yield very different values when the variable order is biased, for example, when the largest value is always presented first, which inflates correlations.
Here we develop a new correlation estimator that is derived under the assumptions of this data, namely that the order of the elements of the pair is arbitrary, and results in an estimator that is invariant to swapping the two values in each pair.
The key assumption implies that the two variables have identical marginal distributions, hence there is more symmetry in this correlation model, which we term ``corsym''.
This estimator is recommended for estimating correlations between parental ancestry proportions, which can be seen as evidence of assortative mating.
