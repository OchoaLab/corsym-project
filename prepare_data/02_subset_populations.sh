...
awk "$REF_POPS { print }" ${IN_FILE_REF}.psam > ${WORK_DIR}_ref_keep.ids
plink2 --bfile ${IN_FILE_REF} --keep ${WORK_DIR}_ref_keep.ids --make-pgen --out ${IN_FILE_REF}_popsubset

###Filter out non-target populations from the trio plink2 file
awk "$TEST_POPS { print }" ${IN_FILE_TRIO}.psam > ${WORK_DIR}_trio_keep.ids
plink2 --pfile ${IN_FILE_TRIO} --keep ${WORK_DIR}_trio_keep.ids --make-pgen --out ${IN_FILE_TRIO}_popsubset
